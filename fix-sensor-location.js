...
async function fixSensorLocation() {
  ...
  try {
    ...
    console.log('=== Checking Sensor 23 Current Data ===');
    const [sensors] = await connection.execute('SELECT id, name, map_id, building_id, floor_id FROM sensors WHERE id = 23');
    const sensor = sensors[0];
    console.log('Sensor 23:', sensor);

    if (sensor?.map_id) {
      // Get the correct location from map_id
      const [maps] = await connection.execute('SELECT id, name, floor_id FROM maps WHERE id = ?', [sensor.map_id]);
      const map = maps[0];
      console.log('Current map:', map);

      if (map?.floor_id) {
        const [floors] = await connection.execute('SELECT id, name, level, building_id FROM floors WHERE id = ?', [map.floor_id]);
        ...
        if (floor?.building_id) {
          ...
          if (sensor.floor_id !== floor.id || sensor.building_id !== floor.building_id) {
            ...
            await connection.execute(
              'UPDATE sensors SET building_id = ?, floor_id = ? WHERE id = 23',
            ...
            // Verify update
            const [updatedSensors] = await connection.execute('SELECT id, name, map_id, building_id, floor_id FROM sensors WHERE id = 23');
